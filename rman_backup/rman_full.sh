...
export SUMMARY_LOG="${BACKUP_DIR}/log/rman_full_${SID}.log"
...
run {
    ...
    backup as compressed backupset database format '${BACKUP_DIR}/%d_full_%s_%p__${RUN_ID}' filesperset 8 tag '${SID}_DB_${DT}';
    ...
    backup as compressed backupset current controlfile format '${BACKUP_DIR}/%d_ctrl_%s_%p__${RUN_ID}' tag '${SID}_CTRL_${DT}';
    backup as compressed backupset spfile format '${BACKUP_DIR}/%d_spfile_%s_%p__${RUN_ID}' tag '${SID}_SPFILE_${DT}';
...
"$AZCOPY_LOCATION" sync "$BACKUP_DIR" "$AZ_BLOB_DEST" \
